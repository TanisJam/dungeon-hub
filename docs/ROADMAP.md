...
|---|---|---|---|
| 0 | **`dm-player-play-model` arc — Slices B/C/D** (Slice A ✅ shipped) | **B**: session play-loop web UI (create/join-with-PJ/leave/run/complete — the missing PLAY surface; all APIs exist, needs E2E). **C**: world-scoped lens + de-entangle (per-world `dh:role`, RoleSwitcher audit, codex/encuentros/worlds gates). **D**: IA tune (Crónica→Bitácora, session CTAs on Inicio). Proposal #1901. | #3.6 DM campaigns + WM playability |
