...
// ---- Starting Equipment schemas (REQ-SEQUIP-04, ADR-5, ADR-6) ---------------

/**
 * Item pick shape for category selections.
 * Key is the category-ref id (e.g. "row1-a-cat0" — generated by the domain resolver).
 */
...
/**
 * REQ-SEQUIP-10 / ADR-6: PUT /characters/:id/equipment-selections body.
 * Persists the player's draft selections to character.data.equipmentSelections.
 */
const EquipmentSelectionsBody = z.object({
  classPath: z.enum(['package', 'gold']),
  goldValue: z.number().int().nonnegative().optional(),
  classRowChoices: z.record(z.string(), z.enum(['a', 'b', 'c'])).default({}),
  classCategoryPicks: z.record(z.string(), CategoryPickSchema).default({}),
  backgroundRowChoices: z.record(z.string(), z.enum(['a', 'b', 'c'])).default({}),
  backgroundCategoryPicks: z.record(z.string(), CategoryPickSchema).default({}),
});

/**
 * REQ-SEQUIP-04 / ADR-5: POST /characters/:id/seed-equipment body.
 * Inline selections for the grant — mirrors EquipmentSelections from domain shape.ts.
 * goldValue must be non-negative when classPath === 'gold' (REQ-SEQUIP-06).
 */
const SeedEquipmentBody = z
  .object({
    classPath: z.enum(['package', 'gold']),
    goldValue: z.number().int().nonnegative().optional(),
    classRowChoices: z.record(z.string(), z.enum(['a', 'b', 'c'])).default({}),
  ...
  .refine(
    (b) => b.classPath !== 'gold' || b.goldValue !== undefined,
    { message: 'goldValue is required when classPath is "gold"' },
  );
...
export const charactersRoute: FastifyPluginAsync = async (app) => {
  ...
  // Persists the player's draft selections to character.data.equipmentSelections.
  // Owner-gated, draft-only (assertWritableForEdit).
  // REQ-SEQUIP-10, ADR-6: selection state round-trips on wizard back-navigation.
  app.put(
    '/characters/:id/equipment-selections',
    { preHandler: app.authenticate },
    async (request, reply) => {
      const { id } = ParamsWithId.parse(request.params);
      const body = EquipmentSelectionsBody.safeParse(request.body);
      if (!body.success) {
        return reply.code(400).send({ error: 'VALIDATION_FAILED', issues: body.error.issues });
      }
      const userId = request.user!.sub;
      ...
      if (!writable.ok) {
        ...
        return reply.code(403).send({ error: 'FORBIDDEN', message: 'Only the owner can edit' });
      }

      const data = (character.data as Record<string, unknown> | null) ?? {};

      // Coerce Zod Record<string, 'a'|'b'|'c'> to Record<number, 'a'|'b'|'c'> at runtime.
      // Keys arrive as strings over JSON; the domain type uses number keys but JS
      // objects always have string keys — the coercion is transparent at runtime.
      const equipmentSelections = {
        classPath: body.data.classPath,
        ...(body.data.goldValue !== undefined ? { goldValue: body.data.goldValue } : {}),
        classRowChoices: body.data.classRowChoices as Record<number, 'a' | 'b' | 'c'>,
        classCategoryPicks: body.data.classCategoryPicks,
        backgroundRowChoices: body.data.backgroundRowChoices as Record<number, 'a' | 'b' | 'c'>,
        backgroundCategoryPicks: body.data.backgroundCategoryPicks,
      };

      await db
        .update(characters)
        .set({
          data: { ...data, equipmentSelections },
          updatedAt: new Date(),
        })
        .where(eq(characters.id, id));
  ...
  // Grants starting equipment to the character (REQ-SEQUIP-04, ADR-5).
  // Idempotent via data.startingEquipmentGranted flag (REQ-SEQUIP-05).
  // Owner-gated. Reads selections from the body (sent by review/publish action).
  app.post(
    '/characters/:id/seed-equipment',
    { preHandler: app.authenticate },
    async (request, reply) => {
      const { id } = ParamsWithId.parse(request.params);
      const body = SeedEquipmentBody.safeParse(request.body);
      if (!body.success) {
        return reply.code(400).send({ error: 'VALIDATION_FAILED', issues: body.error.issues });
      }
      const userId = request.user!.sub;
      ...
      // Owner-gate: only the character owner can seed equipment
      const access = await getCharacterAccess(character, userId);
      if (access !== 'owner') {
        return reply.code(403).send({ error: 'FORBIDDEN', message: 'Only the owner can seed equipment' });
      }

      // Coerce row keys to number (same pattern as equipment-selections)
      const selections = {
        classPath: body.data.classPath,
        ...(body.data.goldValue !== undefined ? { goldValue: body.data.goldValue } : {}),
        classRowChoices: body.data.classRowChoices as Record<number, 'a' | 'b' | 'c'>,
      ...
      const result = await grantStartingEquipment(id, selections);
      if (!result.ok) {
        return reply.code(400).send({ error: 'VALIDATION_FAILED', issues: result.issues });
      }
