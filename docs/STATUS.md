...
| **`compendium-import` silently skips 34 of its 112 tests in CI, every run.** Confirmed by running the suite: `Test Files 6 passed \| 3 skipped (9)`, `Tests 78 passed \| 34 skipped (112)`. Three files skip entirely — `fluff.integration.test.ts`, `seed-pack.smoke.test.ts`, `resolve-copy.integration.test.ts` — via `describe.skipIf(!existsSync(DATA_DIR))`; a fourth, `importers/races.test.ts`, skips only its own real-data describe block (7 of its 14 tests) while its unit tests still run. `data/5etools/` is gitignored ("not versioned and NOT a submodule: populate it manually", per `.gitignore` and `README.md`), so on a fresh clone — including CI, which never runs `fetch-5etools.sh` for this package's own suite — these never execute. The importer is the boundary CLAUDE.md §10 says "has bugs," and its real-data integration tests currently prove nothing except on a developer machine that happened to populate the directory. | Medium | cross-cutting (`compendium-import`, CLAUDE.md §10) |
