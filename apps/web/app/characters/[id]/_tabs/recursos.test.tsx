...
// --- Paladin Lay on Hands (pool-shaped) ---

import * as actions from '../actions';

const layOnHandsL5: ClassResourceView = {
...
describe('RecursosTab — Paladin Lay on Hands (pool-shaped)', () => {
  it('renders Spanish label + class label', () => {
    render(
      <RecursosTab
        characterId="char-1"
        classResources={{ 'paladin:lay-on-hands': layOnHandsL5 }}
      />,
    );
    expect(screen.getByText('Imposición de Manos')).toBeTruthy();
    expect(screen.getByText(/Paladín/)).toBeTruthy();
  });

  it('renders amount input for pool-shaped resources', () => {
    render(
      <RecursosTab
        characterId="char-1"
        classResources={{ 'paladin:lay-on-hands': layOnHandsL5 }}
    ...
    const input = screen.getByTestId('resource-amount-input') as HTMLInputElement;
    expect(input).toBeTruthy();
    expect(input.type).toBe('number');
    expect(input.value).toBe('1');
  });

  it('does NOT render amount input for counter-shaped resources', () => {
    render(
      <RecursosTab
        characterId="char-1"
        classResources={{ 'fighter:second-wind': secondWind }}
    ...
    expect(screen.queryByTestId('resource-amount-input')).toBeNull();
  });

  it('Usar with amount=5 calls useClassResource(id, slug, 5)', async () => {
    render(
      <RecursosTab
        characterId="char-1"
        classResources={{ 'paladin:lay-on-hands': layOnHandsL5 }}
    ...
    const usar = screen.getByText('Usar');
    fireEvent.click(usar);
    expect(actions.useClassResource).toHaveBeenCalledWith('char-1', 'paladin:lay-on-hands', 5);
  });
});
