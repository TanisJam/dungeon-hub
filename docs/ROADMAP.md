...
| 5 | **Map tiles 500 in production** | The live map draws POIs over an empty background: tile requests to Supabase Storage answer `500 {"error":"Internal"}`. Storage is healthy (a missing object still answers `404 not_found`), so the object rows exist but their bytes cannot be served — a home-lab data issue, likely fixed by re-running `apps/api/scripts/upload-map-tiles.ts`. Replaces the former "mobile zoom buttons + waypoint visibility" row: both were verified on 2026-09-07 to be non-issues (zoom buttons render at 390px; the hybrid status-gate visibility model already exists in `schema.ts:459-465`). | #3.5 Map |
