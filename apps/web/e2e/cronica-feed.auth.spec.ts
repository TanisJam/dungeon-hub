...
/**
 * cronica-feed — E2E spec for the unified Bitácora del Gremio feed.
 *
 * bitacora-gremio W4, REQ-GREM-FD-01, REQ-GREM-FD-02, REQ-GREM-FD-03,
 * REQ-GREM-FD-04, REQ-GREM-FD-05, REQ-GREM-E2E-01.
 *
 * Verifies:
 *   (a) /cronica renders the unified feed WITHOUT redirecting (ADR-4).
 ...
 *   (h) Empty state: /cronica with no world → V3Empty shown, no crash.
 *
...
test('/cronica renders unified feed without redirect (REQ-GREM-FD-01, ADR-4)', async ({ page }) => {
  await page.goto('/cronica', { waitUntil: 'domcontentloaded' });

  // URL must stay at /cronica — no redirect to /cronica/eventos
...
test('Tag filter chip strip is visible at /cronica (REQ-GREM-FD-03)', async ({ page }) => {
  await page.goto('/cronica', { waitUntil: 'domcontentloaded' });
  await expect(page).toHaveURL(/\/cronica$/, { timeout: 10_000 });
...
test('Tap "Aportar" → ContributionComposer opens with tags picker (REQ-GREM-FD-05 scenario 2)', async ({ page }) => {
  await page.goto('/cronica', { waitUntil: 'networkidle' });
  await expect(page).toHaveURL(/\/cronica$/, { timeout: 10_000 });
...
test('SubNav shows 3 items: Todo | Eventos | Notas at /cronica (ADR-4)', async ({ page }) => {
  await page.goto('/cronica', { waitUntil: 'domcontentloaded' });
  await expect(page).toHaveURL(/\/cronica$/, { timeout: 10_000 });
...
test('/cronica/eventos deep-link preserved — renders as source-facet (REQ-GREM-FD-04)', async ({ page }) => {
...
test('/cronica/notas deep-link preserved — renders as source-facet (REQ-GREM-FD-04)', async ({ page }) => {
