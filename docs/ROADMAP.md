...
|---|---|---|---|
| 0 | **`dm-player-play-model` arc — Slices C/D** (A ✅ + B ✅ shipped) | **C**: world-scoped lens + de-entangle (per-world `dh:role`, RoleSwitcher/AppShell `callerRole` audit, codex/encuentros/worlds gates). **D**: IA tune (Crónica→Bitácora, session CTAs on Inicio). Proposal #1901. | #3.6 DM campaigns + WM playability |
