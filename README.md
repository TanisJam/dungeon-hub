...
# The 5etools dataset is not committed and is not a submodule. This fetches it — a
# blobless sparse clone of the upstream mirror, ~112 MB — into data/5etools/data/,
# in the layout packages/compendium-import/src/reader.ts expects. No-op if already
# present. Without it, import:5etools fails and so does the API integration suite.
...
- **5etools dataset is not distributed with this repo, but fetching it is automated.** `data/5etools/` is gitignored and there is no submodule, so a fresh clone has no compendium data and `import:5etools` (step 5 above) fails without it — as does the `apps/api` integration suite, which builds characters out of real compendium rows. Run `./scripts/fetch-5etools.sh`: it takes a blobless sparse clone of [`5etools-mirror-3/5etools-src`](https://github.com/5etools-mirror-3/5etools-src), copies only its `data/` tree (~112 MB) into place, and is a no-op once present. Pin a different upstream ref with `FIVEETOOLS_REF=<branch|tag|sha>`.
...
`pnpm test` runs every suite, but `apps/api` is overwhelmingly integration tests that
need a live Postgres + GoTrue — 119 of its 126 files. There are two ways around that:

- `scripts/test-stack.sh <command>` brings up a throwaway Postgres + GoTrue on ports
  54321/54322 (Docker required), migrates, applies the custom SQL, imports the
  compendium, runs your command, and tears the stack down on exit. It never touches
  your own Supabase stack.
- `pnpm --filter @dungeon-hub/api test:unit` skips them and runs only the 7 files that
  need no infrastructure — what CI's fast lane uses.
...
pnpm --filter @dungeon-hub/api test:unit

# The API integration suite (needs Docker; fetch the dataset once, first)
./scripts/fetch-5etools.sh
