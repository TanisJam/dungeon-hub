...
/**
 * bitacora-notas — E2E spec for the Notas source-facet in the Bitácora tab.
 * REQ-CRO-01, REQ-CRO-03, REQ-GATE-01, REQ-GATE-03, REQ-GREM-FD-04.
 * REQ-RENAME-01 (barrido-final): renamed from cronica-notas → bitacora-notas; /cronica → /bitacora.
 *
 * bitacora-gremio W4 changes:
 *   - UPDATED: SubNav now has 3 items: Todo | Eventos | Notas (not 2).
 *   - KEPT: /bitacora/notas deep-link preserved; DM FAB and create-note flow retained (ADR-7, T-19).
 *   - ADDED: assertion that /bitacora/notas shows journal entries via GuildBitacoraFeed (source=dm badge).
 *
 * Verifies:
 *   (a) /bitacora/notas renders (sub-route works — deep-link preserved as source-facet).
...
test('DM view: FAB "Crear" is visible at /bitacora/notas', async ({ page }) => {
  await page.goto('/bitacora/notas', { waitUntil: 'domcontentloaded' });
  await expect(page).toHaveURL(/\/bitacora\/notas/, { timeout: 10_000 });
...
test('DM can create a note via FAB', async ({ page }) => {
  ...
  await page.goto('/bitacora/notas', { waitUntil: 'networkidle' });
  await expect(page).toHaveURL(/\/bitacora\/notas/, { timeout: 10_000 });
...
test('SubNav: clicking Eventos pill navigates to /bitacora/eventos', async ({ page }) => {
  await page.goto('/bitacora/notas', { waitUntil: 'domcontentloaded' });
  await expect(page).toHaveURL(/\/bitacora\/notas/, { timeout: 10_000 });
