        ...
        <div className="px-4 py-3">
          ...
          {!loading && pois !== null && (
            <>
              ...
              {/* Inline create/edit form — DM only; uses shared PoiForm (REQ-PWC-FORM-01) */}
              {isDM && form.open && (
                <PoiForm
                  mode={form.mode}
                  initial={form.initial}
                  idPrefix={hexId}
                  onSubmit={async (body: PoiBody) => {
                    if (form.mode === 'create') {
                      return onCreatePoi(hexId, body);
                    } else if (form.initial) {
                      return onUpdatePoi(form.initial.id, body);
                    }
